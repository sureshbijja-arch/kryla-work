...
export default function MediaTab({ providerId, slug, firstName, plan, onUpgrade }: Props) {
  ...
  useEffect(() => {
    Promise.all([
      supabase.from('providers').select('avatar_url').eq('id', providerId).single(),
      supabase.from('pages').select('gallery').eq('provider_id', providerId).single(),
    ]).then(([avatarRes, galleryRes]) => {
      setAvatarUrl(avatarRes.data?.avatar_url ?? null)
  ...
  if (!canUpload) {
    return (
      <div className="flex-1 flex flex-col items-center justify-center px-6 py-12 text-center">
        ...
        <p className="text-sm text-[#666] mb-5 max-w-xs">Upload your profile photo and gallery. Available on Grow and above.</p>
        <button
          onClick={onUpgrade}
