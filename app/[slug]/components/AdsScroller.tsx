...
export default function AdsScroller({ slug }: { slug: string }) {
  ...
  useEffect(() => {
    fetch(`/api/ads/list?slug=${encodeURIComponent(slug)}`)
      .then(r => r.json())
      .then(d => { if (mounted.current && Array.isArray(d.ads)) setAds(d.ads) })
      .catch(() => {})
  }, [slug])
  ...
  const fade  = { opacity: visible ? 1 : 0, transition: 'opacity 0.22s ease' } as const
  ...
  return (
    <>
      ...
      <div className="hidden xl:flex fixed right-6 top-1/2 -translate-y-1/2 z-40 flex-col items-center gap-2 w-44">
        ...
        <div className="w-full bg-white border border-[#E5E5E5] rounded-xl shadow-sm overflow-hidden" style={fade}>
          {ad.imageUrl && (
      ...
      <div
        className="xl:hidden fixed bottom-0 left-0 right-0 z-40 bg-white border-t border-[#E5E5E5] px-4 py-3 safe-area-bottom"
        style={fade}
      >
        <div className="max-w-2xl mx-auto flex items-center gap-3">
          ...
          {ad.linkUrl && (
            <a href={ad.linkUrl} target="_blank" rel="noopener noreferrer sponsored"
              className="shrink-0 text-[11px] font-semibold text-white bg-[#F5A623] px-3 py-1.5 rounded-lg">
              View
            </a>
          )}
          {multi && <Dots />}
